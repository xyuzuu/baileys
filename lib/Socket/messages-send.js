...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, delay, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, generateWAMessageFromContent, preparePhotoLiveContent, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, hasValidAlbumMedia, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, shouldIncludeBizBinaryNode, unixTimestampSeconds } from '../Utils/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                return fullMsg;
            ...
            else if (typeof content === 'object' && 'image' in content && content.image && 'motion' in content && content.motion && 'photoLive' in content && content.photoLive) {
                 const photoLiveMsg = await preparePhotoLiveContent(jid, content, {
                     relayMessage: await relayMessage,
                     waUploadToServer: await waUploadToServer, 
                 ...
                 if (config.emitOwnEvents) {                       
                     process.nextTick(() => {
                         messageMutex.mutex(() => (upsertMessage(photoLiveMsg, 'append'))) 
                     })
                 }             
                 return photoLiveMsg
            }
