...
const prepareNativeFlowButtons = (message) => {
    ...
    if (hasOptionalProperty(message, 'offerText') && !!message.offerText) {
        Object.assign(messageParamsJson, {
            limited_time_offer: {
                text: message.offerText || 'Baileys',
                url: message.offerUrl || 'https://wa.me',
...
export const preparePhotoLiveContent = async (jid, message, options) => {
    if (!('image' in message || 'motion' in message)) throw new Error('No content in message')
    let mediaHandle
    const imgMsg = await generateWAMessage(jid, {
        ...message,
        image: message.image,
        contextInfo: {
            ...(message.contextInfo || {}),
            ...(message.mentions ? { mentionedJid: message.mentions } : {}),
            ...(message.mentionsAll ? { nonJidMentions: 1 } : {}),
            pairedMediaType: proto.ContextInfo.PairedMediaType.MOTION_PHOTO_PARENT,
            statusSourceType: proto.ContextInfo.StatusSourceType.IMAGE,
        }
    }, {
        userJid: options.userJid,
        upload: async (encFilePath, opts) => {
            const up = await options.waUploadToServer(encFilePath, { ...opts, newsletter: isJidNewsletter(jid) })
    ...
    await options.relayMessage(jid, imgMsg.message, { messageId: imgMsg.key.id, ...options })
    const vidMsg = await generateWAMessage(jid, {
        ...message,
        video: message.motion,
        contextInfo: {
            ...(message.contextInfo || {}),
            ...(message.mentions ? { mentionedJid: message.mentions } : {}),
            ...(message.mentionsAll ? { nonJidMentions: 1 } : {}),
            pairedMediaType: proto.ContextInfo.PairedMediaType.MOTION_PHOTO_CHILD,
            statusSourceType: proto.ContextInfo.StatusSourceType.IMAGE,
        }
    }, {
        userJid: options.userJid,
    ...
    vidMsg.message.messageContextInfo = {
    ...
    await options.relayMessage(jid, vidMsg.message, { messageId: vidMsg.key.id, ...options })
    return imgMsg
};
...
async function prepareStickerPackMessage(stickerPack, options) {
    ...
    const stickerPromises = stickers.map(async (s, i) => {
        try {
            ...
            if (webpBuffer.length > 1024 * 1024) throw new Boom(`Sticker at index ${i} exceeds the 1MB size limit`, { statusCode: 400 });
 ...
 */
export const shouldIncludeBizBinaryNode = (message) => !!(message.buttonsMessage || message.listMessage || message.templateMessage || (message.interactiveMessage && message.interactiveMessage.nativeFlowMessage));
